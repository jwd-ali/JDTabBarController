...
public protocol JTabBarDelegate: AnyObject {
    func tabBar(_ tabBar: JTabBar, didSelectTabAt index: Int)
}

public class JTabBar: UIView {
...
//    }
    ...
    public var colors: [UIColor?] = [ #colorLiteral(red: 0.5172309279, green: 0.7993689179, blue: 0.9946866632, alpha: 1), #colorLiteral(red: 1, green: 0.7976374626, blue: 0, alpha: 1), #colorLiteral(red: 1, green: 0.644939661, blue: 0.5219646692, alpha: 1), #colorLiteral(red: 0.3568160534, green: 0.8285983205, blue: 0.892596066, alpha: 1) ] {
        didSet {
            for (index, item) in stackView.arrangedSubviews.enumerated() {
                if let tabBarItem = item as? JTabBarItem {
    ...
    weak var delegate: JTabBarDelegate?
...
//            .alignEdgesWithSuperview([.left, .right, .top, .bottom], constants:5)
...
extension JTabBar {
    private func drawTabs() {
        for (index , vc) in viewControllers.enumerated() {
            let barView = JTabBarItem(tabBarItem: vc.tabBarItem, color: colors[index] ?? tabBarTintColor, tintColor: .lightGray)
            barView
...
private extension JTabBar {
    ...
    func animateTabItem(index: Int) {
        self.stackView.arrangedSubviews.enumerated().forEach {
            guard let tabView = $1 as? JTabBarItem else { return }
